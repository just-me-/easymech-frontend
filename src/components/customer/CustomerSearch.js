...
function CustomerSearch(props: Props) {
  const initState =
    props.location.state && props.location.state.listRedirect
      ? "list"
      : "search";
  ...
  function onEditCustomerClick(customerId) {
    ...
    api
      ...
      .catch(error => {
        ...
        setViewState("list");
        NotificationManager.error(
          "Beim Laden des Kundens ist ein Fehler aufgetreten.",
          "Bitte erneut versuchen!"
        );
      });
  }
  ...
  return (
    <div>
      ...
      {viewState === "list" && (
        <div>
          <CustomerList
            editCustomer={onEditCustomerClick}
